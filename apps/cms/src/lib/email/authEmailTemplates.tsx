...
export const generateForgotPasswordEmailSubject = ({
  userEmail,
}: Pick<AuthEmailPayload, "userEmail">): string => {
  return `Reset your Sid's Hub CMS password (${userEmail})`;
};

export const generateForgotPasswordEmailHTML = async ({
  resetPasswordURL,
}: AuthEmailPayload): Promise<string> => {
  if (!resetPasswordURL) {
