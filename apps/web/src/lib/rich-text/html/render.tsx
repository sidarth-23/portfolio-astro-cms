...
import { highlightCode } from "@/lib/rich-text/util/shiki";
...
import { createTableConverters } from "../util/table";
import { createInternalDocHrefResolver, type InternalDocHrefRouteMap } from "../util/linkResolver";
import type { CalloutVariantProfile, RichTextRenderOptions, RichTextValue } from "./types";
    ...
    return {
      ...
      upload: async ({ node }: { node: SerializedUploadNode }) => {
        ...
        const alt =
          ...
          (node.value as unknown as PopulatedUploadValue).alt ??
          "";
        const mediaCaption = (node.value as unknown as PopulatedUploadValue).caption;
      ...
      ...createTableConverters({ table: "table not-prose" }),
...
export async function renderBlock(
  ...
  switch (block.blockType) {
    ...
    case "imageGallery": {
      const caption = (block.caption as string | null | undefined) ?? null;
      const rawImages =
        (block.images as Array<{ image: UploadDoc | string | number | null }> | null | undefined) ??
        [];
