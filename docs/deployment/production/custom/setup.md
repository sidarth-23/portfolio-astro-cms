...
Fill in all values. The `.env` file is loaded by `docker-compose.prod.yml` via
...
`docker-compose.prod.yml` does not include any Traefik labels or exposed ports.
...
And update `docker-compose.prod.yml` to add that network to `payload-cms` and
...
```bash
# Dry run first
docker compose -f docker-compose.prod.yml exec payload-cms \
  sh -c "MEDIA_CLEANUP_DRY_RUN=true bun run --filter @sidshub/cms cleanup:media"

# Live run (daily at 3 AM via cron)
# 0 3 * * * docker compose -f /path/to/docker-compose.prod.yml exec -T payload-cms \
