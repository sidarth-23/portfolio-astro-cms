...
export function SocialCardPreview({ siteUrl }: Props) {
  ...
  const { "meta.title": { value: metaTitle } = {}, "meta.image": { value: metaImageValue } = {} } =
    fields as Record<string, { value: unknown }>;
  const metaDescription = (fields as Record<string, { value: unknown }>)["meta.description"]
  ...
  useEffect(() => {
    ...
    const initialImage = (docInfo.initialData as { meta?: { image?: unknown } } | undefined)?.meta
      ?.image;
  ...
  return (
    <div style={{ marginBottom: "20px" }}>
      ...
      {displayUrl && (
        <div
          style={{ marginTop: "6px", fontSize: "12px", color: "var(--theme-text-subtle, #9A9A9A)" }}
        >
          {displayUrl}
        </div>
      )}
